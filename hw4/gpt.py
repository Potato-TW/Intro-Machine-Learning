...
degree_ = 3  # Set the default degree for the polynomial kernel.
gamma_ = 0.8  # Set the default gamma for the rbf kernel.
...
def gram_matrix(X1, X2, kernel_function):
    ...
    for i in range(n1):
        for j in range(n2):
            gram_matrix[i, j] = kernel_function(X1[i], X2[j])
    return gram_matrix
